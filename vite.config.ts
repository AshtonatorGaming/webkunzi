...
function isLeafletId(id: string): boolean {
  ...
  return (
    bare === "leaflet" ||
    bare === "react-leaflet" ||
    bare.startsWith("react-leaflet/") ||
    bare.startsWith("@react-leaflet/") ||
    /\/node_modules\/leaflet\/.+\.(m?js|cjs)$/.test(bare) ||
    /\/node_modules\/react-leaflet(?:\/|$)/.test(bare) ||
    /\/node_modules\/@react-leaflet\//.test(bare)
  );
}

function isWorldMapId(id: string): boolean {
  const bare = id.split("?", 1)[0] ?? id;
  return /(^|\/)WorldMap\.(t|j)sx?$/.test(bare);
}

function isServerEnvironment(pluginContext: {
  environment?: { name?: string; config?: { consumer?: string } };
}) {
  const env = pluginContext.environment;
  return env?.name === "ssr" || env?.name === "nitro" || env?.config?.consumer === "server";
}
...
function leafletSsrStub(): Plugin {
  return {
    ...
    enforce: "pre",
    resolveId(id) {
      if (!isServerEnvironment(this)) return null;
      if (isLeafletId(id)) return "\0leaflet-ssr-stub";
      return null;
    },
    load(id) {
      if (id !== "\0leaflet-ssr-stub") return null;
      return `
...
function nitroLeafletStubPlugin() {
  return {
    name: "inkunzi-nitro-leaflet-stub",
    resolveId(id: string) {
      if (id.includes(".css")) return null;
      if (isLeafletId(id)) return LEAFLET_STUB;
      if (isWorldMapId(id)) return WORLDMAP_STUB;
      return null;
    },
  };
}
